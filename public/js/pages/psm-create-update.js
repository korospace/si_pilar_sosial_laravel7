...
let status_hide = $("#formCreateUpdatePsm #status_hide").val();
let psm_id = $("#formCreateUpdatePsm #id").val();
...
if (
    ($("#formCreateUpdatePsm #id").val() != null && user_level_id != 1) ||
    status_hide == "nonaktif"
) {
    $("#formCreateUpdatePsm input, #formCreateUpdatePsm select").prop(
        "disabled",
        true
    );
}
...
$("#formCreateUpdatePsm #tempat_tugas_kecamatan").on(
    "autocomplete.select",
    function (evt, item) {
...
$("#formCreateUpdatePsm #tempat_tugas_kelurahan").on(
    "autocomplete.select",
    function (evt, item) {
...
$("#formCreateUpdatePsm").validate({
    ...
    highlight: function (element, errorClass, validClass) {
        ...
        if ($(element).attr("name") == "tempat_tugas_kelurahan_hide") {
            $("#formCreateUpdatePsm #tempat_tugas_kelurahan").addClass(
                "is-invalid"
            );
        }
        if ($(element).attr("name") == "tempat_tugas_kecamatan_hide") {
            $("#formCreateUpdatePsm #tempat_tugas_kecamatan").addClass(
                "is-invalid"
            );
        } else {
    ...
    unhighlight: function (element, errorClass, validClass) {
        ...
        if ($(element).attr("name") == "tempat_tugas_kelurahan_hide") {
            $("#formCreateUpdatePsm #tempat_tugas_kelurahan").removeClass(
                "is-invalid"
            );
        }
        if ($(element).attr("name") == "tempat_tugas_kecamatan_hide") {
            $("#formCreateUpdatePsm #tempat_tugas_kecamatan").removeClass(
                "is-invalid"
            );
        } else {
                        ...
                        Swal.fire({
                            ...
                            if (psmId == null) {
                                if (user_level_id == 1) {
                                    ...
                                    $("#formCreateUpdatePsm input")
                                        .not("#site")
                                        .val("");
                                }
                                $("#formCreateUpdatePsm select")
                                    .val("")
                                    .change();
                            } else {
                                location.reload();
                    ...
                    .catch((data) => {
                        if (data.status == 400) {
                            ...
                            for (const key in errors) {
                                $(`#formCreateUpdatePsm #${key}`).addClass(
                                    "is-invalid"
                                );
                                $(`#formCreateUpdatePsm #${key}-error`).html(
                                    errors[key][0]
                                );
                            }
    ...
    if ($("#formNonAktif").valid()) {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                return $.ajax({
                    type: "PUT",
                    url: `${BASE_URL}/api/v1/psm/update_status`,
                    data: form,
                    cache: false,
                    processData: false,
                    contentType: false,
