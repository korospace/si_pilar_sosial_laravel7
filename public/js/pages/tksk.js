...
function initialDataTableTksk(params) {
    ...
    $("#tableTksk")
        ...
        .buttons()
        .container()
        .appendTo("#tableTksk_wrapper .col-md-6:eq(0)");
    ...
    $("#tableTksk_nonaktif")
        ...
        .buttons()
        .container()
        .appendTo("#tableTksk_nonaktif_wrapper .col-md-6:eq(0)");
}
...
$(".nav-tabs a").on("shown.bs.tab", function (event) {
    ...
    if (href == "#div_nonaktif") {
        $("#formFilterTksk #status_filter_wraper").hide();
    } else {
        $("#formFilterTksk #status_filter_wraper").show();
    }
...
function clear_filter_tksk() {
    if (user_level_id == 1) {
        $("#modal-filter-tksk input").val("");
    } else {
        $(
            "#modal-filter-tksk input:not(#site_filter_id):not(#site_filter)"
        ).val("");
    }
    $("#modal-filter-tksk select").val("").change();
...
$("#download_excel").on("click", function (event) {
    ...
    $.ajax({
        type: "GET",
        url: `${BASE_URL}/api/v1/tksk/download_excel`,
        headers: {
            token: $token,
        },
        success: function (data) {
