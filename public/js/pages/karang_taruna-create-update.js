...
let status_hide = $("#formCreateUpdateKarangTaruna #status_hide").val();
let kt_id = $("#formCreateUpdateKarangTaruna #id").val();
...
if (
    ($("#formCreateUpdateKarangTaruna #id").val() != null &&
        user_level_id != 1) ||
    status_hide == "nonaktif"
) {
    $(
        "#formCreateUpdateKarangTaruna input, #formCreateUpdateKarangTaruna select"
    ).prop("disabled", true);
}
...
function EnableAutoCompleteWilayah() {
    ...
    $("#formCreateUpdateKarangTaruna #site").on(
        "autocomplete.select",
        function (evt, item) {
...
$("#formCreateUpdateKarangTaruna #alamat_kelurahan").on(
    "autocomplete.select",
    function (evt, item) {
        $("#formCreateUpdateKarangTaruna #alamat_kelurahan_hide").val(
            item.text
...
$("#formCreateUpdateKarangTaruna #alamat_kecamatan").on(
    "autocomplete.select",
    function (evt, item) {
        $("#formCreateUpdateKarangTaruna #alamat_kecamatan_hide").val(
            item.text
...
// kepengurusan
$("#formCreateUpdateKarangTaruna #kepengurusan_status").on(
    "change",
    function () {
...
$("#formCreateUpdateKarangTaruna").validate({
    ...
    highlight: function (element, errorClass, validClass) {
        ...
        if ($(element).attr("name") == "alamat_kelurahan_hide") {
            $("#formCreateUpdateKarangTaruna #alamat_kelurahan").addClass(
                "is-invalid"
            );
        }
        if ($(element).attr("name") == "alamat_kecamatan_hide") {
            $("#formCreateUpdateKarangTaruna #alamat_kecamatan").addClass(
                "is-invalid"
            );
        } else {
    ...
    unhighlight: function (element, errorClass, validClass) {
        ...
        if ($(element).attr("name") == "alamat_kelurahan_hide") {
            $("#formCreateUpdateKarangTaruna #alamat_kelurahan").removeClass(
                "is-invalid"
            );
        }
        if ($(element).attr("name") == "alamat_kecamatan_hide") {
            $("#formCreateUpdateKarangTaruna #alamat_kecamatan").removeClass(
                "is-invalid"
            );
        } else {
...
function saveData() {
    if ($("#formCreateUpdateKarangTaruna").valid()) {
        let form = new FormData(
            document.querySelector("#formCreateUpdateKarangTaruna")
        );
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                return $.ajax({
                    type: ktId ? "PUT" : "POST",
                    url: `${BASE_URL}/api/v1/karang_taruna/${
                        ktId ? "update" : "create"
                    }`,
                    data: form,
                    cache: false,
                    ...
                    .then((response) => {
                        Swal.fire({
                            ...
                            if (ktId == null) {
                                if (user_level_id == 1) {
                                    $(
                                        "#formCreateUpdateKarangTaruna input"
                                    ).val("");
                                } else {
                                    $("#formCreateUpdateKarangTaruna input")
                                        .not("#site")
                                ...
                                $("#formCreateUpdateKarangTaruna select")
                                    .val("")
                                    .change();
                            } else {
                                location.reload();
                    ...
                    .catch((data) => {
                        if (data.status == 400) {
                            ...
                            for (const key in errors) {
                                $(
                                    `#formCreateUpdateKarangTaruna #${key}`
                                ).addClass("is-invalid");
                                $(
                                    `#formCreateUpdateKarangTaruna #${key}-error`
                                ).html(errors[key][0]);
                            }
    ...
    if ($("#formNonAktif").valid()) {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                return $.ajax({
                    type: "PUT",
                    url: `${BASE_URL}/api/v1/karang_taruna/update_status`,
                    data: form,
                    cache: false,
                    processData: false,
                    contentType: false,
