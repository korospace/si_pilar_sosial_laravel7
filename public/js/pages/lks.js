...
function initialDataTableLks(params) {
    ...
    $("#tableLks")
        ...
        .buttons()
        .container()
        .appendTo("#tableLks_wrapper .col-md-6:eq(0)");
    ...
    $("#tableLks_nonaktif")
        ...
        .buttons()
        .container()
        .appendTo("#tableLks_nonaktif_wrapper .col-md-6:eq(0)");
}
...
$(".nav-tabs a").on("shown.bs.tab", function (event) {
    ...
    if (href == "#div_nonaktif") {
        $("#formFilterLks #status_filter_wraper").hide();
    } else {
        $("#formFilterLks #status_filter_wraper").show();
    }
...
function clear_filter_lks() {
    if (user_level_id == 1) {
        $("#modal-filter-lks input").val("");
    } else {
        $("#modal-filter-lks input:not(#site_filter_id):not(#site_filter)").val(
            ""
        );
    }
    $("#modal-filter-lks select").val("").change();
...
$("a[data-target='#modal-import-lks']").on("click", function () {
    if (user_level_id == 1) {
        ...
        $("#formImportLks input").not("#site").val("");
        $("#formImportLks #site").prop("disabled", true);
    }
...
$("#download_excel").on("click", function (event) {
    ...
    $.ajax({
        type: "GET",
        url: `${BASE_URL}/api/v1/lks/download_excel`,
        headers: {
            token: $token,
        },
        success: function (data) {
