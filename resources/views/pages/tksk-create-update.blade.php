    ...
    <div class="content mt-4">
		<div class="container-fluid">
            ...
            <div class="row px-2 mb-5" style="display: {{ $tksk != null ? '' : 'none' }}">
                ...
                @if($tksk != null && $tksk->status == "nonaktif" && $status_nonaktif != null)
                <div class="col-12">
                    <p class="text-danger">
            ...
            <div class="row px-2 pb-5">
				<form id="formCreateUpdateTksk" class="col-12" autocomplete="off" style="position: relative;">
                    ...
                    <input type="text" id="status_hide" name="status_hide" value="{{ $tksk->status }}" style="position: absolute;z-index: -10;opacity: 0;max-width: 0px;">
                    @endif
                    ...
                    @elseif ($tksk != null && $user->level_id == 1)
                        @if ($tksk->status != 'diterima' && $tksk->status != 'nonaktif')
                            <button type="button" class="w-100 mt-4 btn btn-success" onclick="saveData()">SIMPAN</button>
                        @elseif ($tksk->status == 'nonaktif')
                            <button type="button" class="w-100 mt-4 btn btn-info" onclick="aktifasi()">AKTIFASI</button>
                        @elseif ($tksk->status == 'diterima')
                            <div class="row">
                                <div class="col-md-6">
                        ...
                        @endif
                    @elseif ($tksk != null && $user->level_id == 2)
                        @if ($tksk->status == 'diterima')
                            <button type="button" class="w-100 mt-4 btn btn-warning" data-toggle="modal" data-target="#modal-non-aktif">NON AKTIFKAN</button>
                        @elseif ($tksk->status == 'nonaktif')
                            <button type="button" class="w-100 mt-4 btn btn-info" onclick="aktifasi()">AKTIFASI</button>
                        @elseif ($tksk->status == 'diperiksa')
                            <div class="row">
                                <div class="col-md-6">
        ...
        {{-- modal non-aktif --}}
        @if ($tksk != null && $tksk->status == 'diterima' && ($user->level_id == 1 || $user->level_id == 2))
